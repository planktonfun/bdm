...
const chatMessageSchema = inferSchema({
  playerId: 255*255,
  message: "string",
  messageType: { type: "enum", values: MessageTypes },
});
applyEnumSchema(chatMessageSchema, 'messageType', MessageTypes);

const successChatSchema = inferSchema({
  playerId: 255*255,
  success: true,
  message: "string",
  timestamp: Date.now(),
...
// a collection of event from the server
const eventCollectionSchema = inferSchema({
  tick: 255*255,
  events: eventObjectsTemplate
});

// sent in initiating/entering/leaving chunks
const snapshotSchema = inferSchema({
  tick: 255*255,
  snapshot: { hunger: 255*255, tick: 255*255, sprites: spriteObjectsTemplate }
});
applyEnumSchema(snapshotSchema, 'spriteName', SpriteTypes);


globalThis.Packer = {
  chatMessagePacker: makeSerializer(chatMessageSchema),
  successChatPacker: makeSerializer(successChatSchema),
  eventPacker: makeSerializer(eventSchema),
  eventCollectionPacker: makeSerializer(eventCollectionSchema),
